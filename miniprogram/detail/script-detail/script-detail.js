...
const SELECT_OPTIONS = {
  // 快件异常情况（场景 1；来自《快递暂行条例》：延误、丢失、损毁、内件短少）
  '快件异常情况': [
    '快件丢失', '快件破损', '运输延误', '内件短少', '派送未联系',
    '被违规投放快递柜/驿站', '其他'
  ],
  ...
  '外卖平台': [
    ...
    '肯德基宅急送', '麦当劳麦乐送', '必胜客宅享送', '商家自营外卖', '其他'
  ],
  // 快递公司（按国内业务量/常见度排序）
  '快递公司名称': [
  ...
  '快递柜或驿站名称': [
    ...
    '兔喜快递超市', '驿收发', '快宝驿站', '多多驿站', '邮政便民服务站', '其他'
  ],

  // ===== 以下为「选项类」占位符：原名是候选列表，改为可点选（避免只能手输、标签退化成「投诉事由」）=====
  // 网约车/出租车司机违规（场景 11）
  '拒载/绕道行驶/不打表/乱收费/态度恶劣/中途甩客': [
    '拒载', '绕道行驶', '不打表', '乱收费', '态度恶劣', '中途甩客', '其他'
  ],
  // 装修施工违规（场景 13）
  '偷工减料/施工质量不合格/无故延期/未经同意增项加价': [
    '偷工减料', '施工质量不合格', '无故延期', '未经同意增项加价', '其他'
  ],
  '质量不达标/已延期X天/仍未完工': ['质量不达标', '已延期', '仍未完工', '其他'],
  // 健身房 / 美容院预付费（场景 12）
  '健身卡/美容卡/会员卡': ['健身卡', '美容卡', '会员卡', '其他'],
  '关门停业/负责人失联/转让后不承认原会员卡': [
    '关门停业', '负责人失联', '转让后不承认原会员卡', '其他'
  ],
  '联系不上/拒绝退款': ['联系不上', '拒绝退款', '拖延处理', '其他'],
  // 网购商家（场景 15）
  '不回复/拒绝退款/拖延处理': ['不回复', '拒绝退款', '拖延处理', '其他'],
  // 噪音扰民（场景 14）
  '夜间施工/高音喇叭/装修噪音/车辆鸣笛': [
    '夜间施工', '高音喇叭', '装修噪音', '车辆鸣笛', '其他'
  ],
  '无法休息/影响学习/身体健康受损': ['无法休息', '影响学习', '身体健康受损', '其他'],
  // 汽车 4S 店（场景 16）
  '强制捆绑保险/装潢/上牌服务/加价提车/售后以次充好/车辆质量问题推诿': [
    '强制捆绑保险', '强制装潢', '强制上牌服务', '加价提车', '售后以次充好', '车辆质量问题推诿', '其他'
  ],

  // 物业问题类型（场景 5）
  '电梯长期故障不修/公共区域照明损坏/垃圾清理不及时/擅自上涨物业费/收取未公示的停车费': [
    '电梯长期故障不修', '公共区域照明损坏', '垃圾清理不及时', '擅自上涨物业费', '收取未公示的停车费', '其他'
  ],
  // 教育培训机构（场景 8）
  '停课/更换老师/缩减课时/负责人失联': ['停课', '更换老师', '缩减课时', '负责人失联', '其他'],
  '拒绝退款/拖延处理/联系不上': ['拒绝退款', '拖延处理', '联系不上', '其他'],
  // 租房（场景 9）
  '租期届满/提前解除合同': ['租期届满', '提前解除合同', '其他'],
  '房东违规行为': ['擅自涨租', '房屋设施损坏不修', '提前收房', '其他'],
  // 外卖 / 餐饮（场景 10）
  '腹痛/腹泻/呕吐': ['腹痛', '腹泻', '呕吐', '其他不适症状'],
  // 医疗（场景 7）
  '本人/家属': ['本人', '家属'],
  '就诊/住院': ['就诊', '住院'],
  '损害后果': ['经济损失', '身体损害', '精神痛苦', '其他'],
  // 银行 / 保险（场景 4）
  '存款/理财': ['存款', '理财'],
  '保险/高风险产品': ['保险产品', '高风险理财产品'],
  // 网购（场景 3 / 15）
  '商品问题情况': ['商品与宣传不符', '存在质量问题', '涉嫌假冒伪劣', '其他'],
  '商家违约情况': ['仍未发货', '虚假发货（物流无记录）', '发空包', '其他'],
  // 噪音（场景 14）
  '噪音来源': ['建筑工地', '商铺', '邻居', '道路/管理单位', '其他'],
  // 通用话术
  '未得到解决/处理结果不满意/对方态度恶劣': ['未得到解决', '处理结果不满意', '对方态度恶劣', '其他'],
  '办理业务/购买商品/接受服务': ['办理业务', '购买商品', '接受服务'],
  '消费者权益/公共利益/市场秩序': ['消费者权益', '公共利益', '市场秩序'],
  '消费者权益/市场秩序/公共利益': ['消费者权益', '市场秩序', '公共利益'],
  '具体诉求一，如退款/赔偿/整改/道歉': ['退款', '赔偿', '整改', '道歉', '其他'],
  '具体诉求一，如退还费用XX元/赔偿损失XX元/立即整改/书面道歉': [
    '退还费用', '赔偿损失', '立即整改', '书面道歉', '其他'
  ],
  '具体描述违法行为，如价格欺诈/虚假宣传/无证经营/安全生产隐患/偷税漏税等': [
    '价格欺诈', '虚假宣传', '无证经营', '安全生产隐患', '偷税漏税', '其他'
  ]
};
...
Page({
  ...
  getPlaceholderType(name) {
    ...
    // 数字类：金额 / 分贝 / 天数 / 课时 / 次数等 → 弹出数字键盘（digit，支持小数点）
    // 注意排除「诉求」等自由文本（标签含诉求，且内容含「退还费用XX元」这类说明）
    if (!/诉求/.test(text) && /金额|元|分贝|天数|课时|次数|月数|年限|持续/.test(text)) return 'number';
    return 'text';
  },
  ...
  onSelectChange(e) {
    ...
    if (value === '其他') {
      formData[field] = '';
      this.setData({ formData });
      wx.showToast({ title: '请在输入框填写具体情况', icon: 'none', duration: 2200 });
      return;
    }
