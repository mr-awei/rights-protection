...
const KEYWORDS = {
  domain: {
    ...
    // ===== 房产物业（35个）=====
    '房产物业': ['物业', '房产', '租房', '房东', '业主', '电梯', '小区', '物业费', '房屋质量', '中介', '房租', '押金', '房产证', '交房', '验房', '装修', '开发商', '楼盘', '房价', '公摊面积', '停车位', '车位费', '供暖', '暖气', '自来水', '水压', '停电', '电路故障', '楼道卫生', '小区绿化', '安保', '门禁', '业委会'],
    // ===== 劳动用工（35个）=====
    ...
    // ===== 政务纪检（25个）=====
    '政务纪检': ['政府', '公职', '官员', '不作为', '推诿', '纪检', '举报', '信访', '督查', '办事', '审批', '政务服务', '行政复议', '行政诉讼', '国家赔偿', '信息公开', '政务公开', '公务员', '事业单位', '编制', '吃空饷', '贪污', '受贿', '腐败', '滥用职权'],
    // ===== 网络安全（30个）=====
  ...
  issue: {
    ...
    // ===== 服务态度类（12个）=====
    '服务态度': ['态度差', '骂人', '凶', '不耐烦', '拒绝服务', '挂电话', '辱骂', '态度恶劣', '不礼貌', '不尊重', '歧视', '差别对待'],
    // ===== 安全类（12个）=====
    '安全': ['诈骗', '被骗', '盗刷', '偷钱', '威胁', '恐吓', '人身安全', '被打', '安全隐患', '危险', '不安全', '风险'],
    // ===== 质量类（12个）=====
    '质量': ['质量差', '有问题', '故障', '不达标', '不合格', '次品', '劣质', '做工差', '材料差', '不耐用', '容易坏', '质量问题'],
    // ===== 合同类（12个）=====
    '合同': ['违约', '不履行', '霸王条款', '格式条款', '单方面变更', '毁约', '合同纠纷', '不按合同', '违反约定', '承诺不兑现', '说话不算数', '反悔'],
    // ===== 虚假宣传类（新增，10个）=====
    '虚假宣传': ['虚假宣传', '夸大宣传', '误导宣传', '虚假广告', '欺骗消费者', '虚假描述', '图文不符', '实物与描述不符', '虚假承诺', '夸大效果'],
    // ===== 强制交易类（新增，10个）=====
    '强制交易': ['强制消费', '强制购买', '强制搭售', '捆绑销售', '强制开通', '强制续费', '自动续费', '默认勾选', '强制升级', '霸王条款'],
    // ===== 隐私泄露类（新增，10个）=====
    '隐私泄露': ['个人信息泄露', '隐私泄露', '信息被卖', '骚扰电话多', '垃圾短信多', '信息被盗用', '身份信息泄露', '电话号码泄露', '住址泄露', '被人肉'],
    // ===== 卫生问题类（新增，10个）=====
    '卫生问题': ['不卫生', '脏乱差', '卫生差', '不干净', '有异味', '有虫子', '有蟑螂', '有老鼠', '消毒不彻底', '卫生不达标']
  },
  target: {
    // ===== 快递公司（10个）=====
...
function extractKeywords(text) {
  ...
  for (const scene of SCENES) {
    ...
    if (scene.matchMode === 'all' && matchCount === scene.keywords.length) {
      matchedScenes.push({ ...scene, matchScore: scene.priority + matchCount * 10 });
