...
| 最后更新 | 2026-09-05 |
| 产品阶段 | MVP（V1.0） |
| 配套文档 | 《我不能被欺负 · 产品需求文档（PRD）》《我不能被欺负 · 产品远景规划》 |
...
| v1.1.0 | 2026-09-09 | — | 配套正式版 PRD v1.1.0：裁剪社区生态预留表（posts/comments/likes），更新 PRD 引用 |
...
| 格式 | 优势 | 劣势 | 结论 |
|------|------|------|------|
| **JS（module.exports）+ 轻量索引 + 按需分片** | 小程序require加载100%成功、解析快、启动仅加载56KB索引、详情按需加载60KB分片、单文件永不超500KB、支持扩展到2000+条 | 数据文件需用JS格式（非标准JSON） | ✅ **选用** |
| JSON | 标准格式、易读易改 | 小程序require JSON偶发加载失败（基础库兼容性问题）、单文件超500KB时加载失败、全量加载内存占用高 | ❌ 不适合小程序运行时 |
...
**决策理由**：JS格式是小程序端最优选择。require JS文件100%加载成功（无基础库兼容性问题），采用"轻量索引+按需分片"架构：启动仅加载56KB索引（含id/name/phone/tags/part_num），搜索用索引数据，点击详情时按需加载对应分片（每片50条约60KB）。单文件永不超500KB，扩展到2000条只需40个分片（各约240KB），内存占用可控。
...
```
Word文档（源数据，持续更新）
    ↓ 【转换脚本 convert.py】首次提取 / 增量更新
Excel文件（维护层，人工审核编辑，多sheet）
    ↓ 【转换脚本 convert.py --mode build】构建+校验
JS分片数据（运行层，小程序打包使用）
    ├── channels_index.js    （轻量索引，56KB，启动加载）
    ├── channels_part_1.js   （分片1，50条，60KB，按需加载）
    ├── channels_part_2.js   （分片2，50条，60KB，按需加载）
    ├── channels_part_N.js   （分片N，按需扩展）
    ├── channels_config.js   （分片配置）
    ├── scripts.js           （话术，全量，数量少）
        ├── config.js            （应用配置）
    └── categories.js        （分类树）
    ↓ 打包
微信小程序（运行时加载索引，搜索用索引，详情按需加载分片）
```

**工作流**：
- **首次**：Word → convert.py --mode extract → Excel（人工审核）→ convert.py --mode build → JS分片数据
- **日常更新（编辑Excel）**：修改Excel → convert.py --mode build → JS分片数据（自动重新分片）
- **日常更新（更新Word）**：更新Word → convert.py --mode extract --diff → 人工确认 → Excel → convert.py --mode build → JS分片数据

**分片规则**：
- 每分片50条渠道（约60KB）
- 索引只含 id/name/phone/tags/part_num/category（56KB）
- 渠道数增长时自动新增分片，无需改代码
- 扩展到2000条只需40个分片（各约240KB），单文件永不超500KB

### 2.3 搜索方案选型

| 方案 | 优势 | 劣势 | 结论 |
|------|------|------|------|
| **本地关键词提取+场景选项卡+索引名称匹配** | 零API成本、响应极快（<20ms）、离线可用、可控性强、无依赖、用户可看到匹配关键词透明可信 | 需维护关键词词库（330+词）、自然语言理解能力有限 | ✅ **选用（MVP）** |
| 本地倒排索引（JS实现） | 离线可用、响应快、支持全文检索 | 需手写索引构建、索引文件大（696KB超500KB限制）、数据量受限 | ⚠️ 已实现但因单文件超限弃用，保留代码 |
...
| AI大模型语义理解 | 自然语言理解能力强、可处理千奇百怪的描述 | 需要API费用（用户多了每月成本高）、依赖网络、不可解释、有延迟 | ⚠️ V2.0可选接入（开发者自选厂商/协议/模型），MVP不接入 |

**决策理由**：MVP采用纯本地关键词提取+场景选项卡方案，零API成本。用户输入一大段自然语言后，系统用正向最大匹配从330+关键词词库中提取核心关键词（如"快递""丢失"），匹配到2-5个相关场景，展示场景选项卡让用户点选。搜索用轻量索引（56KB）做名称匹配+关键词权重，不加载详细数据。AI完全可选，架构预留接入能力，开发者可自行选择厂商（豆包/通义千问/OpenAI等）、协议（OpenAI兼容/Anthropic等）和模型。
...
| 样式 | WXSS + Flex布局 | — | 样式 |
| 运行时数据 | JS（module.exports）+ 轻量索引 + 按需分片 | — | 渠道/话术/法律/配置数据，启动加载56KB索引，详情按需加载60KB分片 |
| 维护层数据 | Excel (.xlsx) | — | 人工编辑维护 |
| 转换脚本 | Python 3.8+ | — | Word→Excel→JS分片转换，含数据分片、增量diff |
| 搜索 | 本地关键词提取（正向最大匹配）+ 场景选项卡 + 索引名称匹配 | — | 330+关键词词库，20预设场景，零API成本 |
...
│  │                    数据层 (Data)                          │    │
│  │  channels_index.js (轻量索引,56KB,启动加载)               │    │
│  │  channels_part_1/2/N.js (详细数据分片,60KB/片,按需加载)  │    │
│  │  channels_config.js (分片配置)                             │    │
...
│  Word文档(.docx)                                                  │
│      │                                                            │
│      ▼  convert.py --mode extract (Python, python-docx + openpyxl) │
│  Excel文件(.xlsx)  ←── 人工审核/编辑 ──→  diff报告              │
│      │                                                            │
│      ▼  convert.py --mode build (Python, openpyxl + 校验)       │
│  JS分片数据(.js) + Schema校验 + 数据质量报告                      │
│  ├── channels_index.js (轻量索引)                                 │
│  ├── channels_part_1/N.js (按需分片)                              │
│  ├── channels_config.js (分片配置)                                │
│  ├── scripts.js / config.js / categories.js            │
│      │                                                            │
│      ▼  打包进小程序                                              │
│  微信小程序（启动加载索引，搜索用索引，详情按需加载分片）          │
│                                                                   │
└─────────────────────────────────────────────────────────────────┘
```
...
3. **数据冗余**：渠道/话术数据字段重复，需合理分片避免重复加载
...
│  法律依据直接存在渠道/话术数据的 legal_basis 字段中，不独立建库 │
└─────────────────────────────────────────────────────────┘
```

> **设计决策**：MVP阶段不做独立法律法规库和法律去重机制。法律依据直接以文本形式存储在渠道/话术数据的`legal_basis`字段中，详情页直接展示。该方案虽然有少量数据冗余，但大幅降低了数据转换和维护复杂度，适合MVP阶段快速迭代。

**触发条件**：默认开启，无论渠道数多少都采用分片架构（保证未来扩展无需改架构）。

**输出结构**（convert.py --mode build 自动生成）：
```
miniprogram/data/
├── channels_index.js          (轻量索引，全量，常驻内存，56KB)
│   └── 每条只含: id, name, phone, tags, part_num, category
├── channels_config.js          (分片配置: total/num_parts/channels_per_part/part_files)
├── channels_part_1.js          (分片1，50条，60KB，按需加载)
├── channels_part_2.js          (分片2，50条，60KB，按需加载)
├── channels_part_3.js          (分片3，22条，27KB，按需加载)
├── channels_part_N.js          (分片N，自动扩展)
├── channels.js                  (完整数据，兼容旧版，可选保留)
├── scripts.js                   (话术，全量，数量少)
├── config.js                    (应用配置)
├── categories.js                (分类树)
...
**小程序端加载策略**（utils/data.js 已实现）：
1. 启动时加载 channels_index.js（56KB轻量索引）+ config.js + categories.js + scripts.js
2. 搜索时只查索引数据（名称匹配+关键词权重），不加载详细内容
3. 点击搜索结果/渠道详情时，根据渠道ID的 part_num，按需加载对应 channels_part_N.js
4. 已加载的分片缓存在内存（Map），重复访问<10ms
5. getChannelById() 自动判断并加载对应分片，上层代码无需关心分片逻辑

**体积估算（不同规模）**：
| 渠道数 | 索引大小 | 分片数 | 每片大小 | 启动加载 | 单文件最大 |
|--------|----------|--------|----------|----------|------------|
| 122（当前） | 56KB | 3 | 60KB | 56KB | 60KB |
| 500 | ~200KB | 10 | ~60KB | 200KB | 60KB |
| 1000 | ~400KB | 20 | ~60KB | 400KB | 60KB |
| 2000 | ~800KB | 40 | ~60KB | 800KB | 60KB |

**关键优势**：
- 单文件永不超500KB（微信小程序require限制）
- 启动加载量可控（仅索引，不加载详情）
- 内存占用可控（仅加载已浏览的分片）
- 扩展到2000+条无需改代码，convert.py自动新增分片
- 搜索性能不受数据量影响（只查索引，O(n) n=渠道数）

#### 7.3.5 分阶段演进路线

| 阶段 | 场景数 | 数据架构 | 搜索方式 | 后端 |
|------|--------|----------|----------|------|
| MVP（当前） | <500 | 轻量索引+按需分片 | 本地关键词提取+场景选项卡+索引名称匹配 | 无 |
| V1.5 | 500-1000 | 分片自动扩展（无需改架构） | 本地搜索+模糊匹配+拼音搜索 | 无 |
| V2.0 | 1000-2000 | 分片+云数据库同步 | 本地索引兜底+云函数搜索+可选AI语义 | 微信云开发 |
| V3.0 | 2000+ | 独立后端+ES | API搜索+离线缓存+AI智能路由 | Node.js+Elasticsearch |
...
- AI语义搜索可选接入（开发者自选厂商/协议/模型，config配置）
...
| 数据自动分片 | 默认开启 | 每50条渠道一个分片，输出channels_part_N.js + channels_index.js + channels_config.js |
| 轻量索引构建 | 默认开启 | 索引只含id/name/phone/tags/part_num/category，启动加载仅56KB |
| 预构建倒排索引 | 默认开启 | IndexBuilder类，压缩格式（数字索引代替字符串），输出search_index.json（保留文件，小程序端不再require） |
| 搜索联想Trie树 | 默认开启 | 从名称/标签/关键词构建，输出suggest_trie.json（保留文件） |
| 数据校验 | 默认开启 | 必填/枚举/外键/格式校验 |
| 增量更新(diff) | --diff参数 | 新旧对比，输出新增/修改/删除/冲突报告 |
| JS格式输出 | 默认开启 | 所有运行时数据输出为JS（module.exports），确保require 100%成功 |
...
| 索引加载时间 | < 200ms（仅加载56KB轻量索引） | 启动时计时 |
| 分片按需加载 | < 100ms/片（点击详情时加载60KB分片） | 详情页打开计时 |
| 搜索响应 | < 500ms（P95），< 100ms（P50，基于索引数据搜索） | 50个典型query计时 |
| 页面切换 | < 300ms | 真机体验 |
| 主包体积 | < 1MB | 微信开发者工具体积分析 |
| 单数据文件 | < 500KB（分片架构保证，当前最大60KB） | 文件大小检查 |
| 内存占用 | < 100MB（仅加载已浏览分片） | 真机性能监控 |
| 首屏渲染 | < 1s | 从启动到首页可交互 |
| 数据扩展性 | 支持2000+条渠道（分片自动扩展） | 压力测试 |
...
| 优化项 | 措施 | 预期体积 |
|--------|------|----------|
| 数据分片 | 启动仅加载56KB索引，详情按需加载60KB分片，单文件永不超500KB | 启动加载56KB |
| 数据压缩 | JS去除空格换行（minify） | 分片60KB → 40KB |
| 无第三方库 | 不引入任何npm包，全部自研 | 0KB额外依赖 |
| 图标内联 | 简单PNG图标（81x81），不使用大图 | 6个图标约20KB |
...
数据文件 (JS分片):         ~350KB（索引56KB + 3个分片152KB + 其他约140KB）
图标资源:                  ~20KB
其他:                      ~10KB
─────────────────────────────
总计:                      ~680KB（远低于1MB目标）
...
```
业务代码:                  ~300KB
数据文件:                  索引800KB + 启动不加载分片（按需）
图标资源:                  ~20KB
其他:                      ~10KB
─────────────────────────────
总计:                      ~1.1MB（接近1MB，可通过分包或索引压缩优化）
```
注：2000条时索引约800KB，可考虑将索引也分片或压缩，或放到分包加载。
...
| UGC审核 | 当前无UGC内容；若未来上线社区，采用先审后发，AI初审+人工复审，24小时巡查 |
...
**词库规模**：领域词50+、问题词100+、对象词80+、场景映射100+，当前版本覆盖80%以上用户输入。
