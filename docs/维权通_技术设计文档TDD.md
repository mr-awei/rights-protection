...
| 转换脚本 | Python 3.8+ | — | Word→Excel→JS分片转换，含法律法规去重、数据分片、增量diff |
...
│  │  channels_config.js (分片配置)                             │    │
│  │  scripts.js │ laws.js (去重) │ config.js │ categories.js │    │
...
| 阶段 | 场景数 | 数据架构 | 搜索方式 | 后端 |
|------|--------|----------|----------|------|
| MVP（当前） | <500 | 轻量索引+按需分片+法律法规去重 | 本地关键词提取+场景选项卡+索引名称匹配 | 无 |
...
| 能力 | 触发条件 | 说明 |
|------|----------|------|
| 法律法规自动提取去重 | 默认开启 | LawExtractor类，支持50+常见法律简称映射，去重后分配唯一ID |
